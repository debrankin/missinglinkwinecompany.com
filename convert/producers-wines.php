...
$colname_rs_wines = "-1";
if (isset($_GET['recordsetPRODUCER'])) {
  $colname_rs_wines = $_GET['recordsetPRODUCER'];
}
((bool)mysqli_query( $admin, "USE $database_admin"));
$query_rs_wines = sprintf("SELECT * FROM sitesearch WHERE producer_id = %s", GetSQLValueString($colname_rs_wines, "text"));
$rs_wines = mysqli_query( $admin, $query_rs_wines) or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
$row_rs_wines = mysqli_fetch_assoc($rs_wines);
$totalRows_rs_wines = mysqli_num_rows($rs_wines);$colname_rs_wines = "-1";
if (isset($_GET['recordsetPRODUCER'])) {
  $colname_rs_wines = $_GET['recordsetPRODUCER'];
}
((bool)mysqli_query( $admin, "USE $database_admin"));
...
$rs_wines = mysqli_query( $admin, $query_rs_wines) or die(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)));
$row_rs_wines = mysqli_fetch_assoc($rs_wines);
$totalRows_rs_wines = mysqli_num_rows($rs_wines);
?>
<!doctype html>
 ...
 <input type="submit" name="search" value="Search" />
  ...
  <div class="columns-wrapper">
      ...
      <div class="main-content-wide">
        <div class="content p7ehc-1">
          <h2><?php echo $row_rs_wines['producer']; ?></h2>
          <?php do { ?>
            <div class="repeatregion">
              <div class="label"><?php echo $row_rs_wines['wines']; ?></div>
              <div class="wines-details"><?php echo $row_rs_wines['wines']; ?></div>
              <p><br clear="all" />
              </p>
            </div>
            <?php } while ($row_rs_wines = mysqli_fetch_assoc($rs_wines)); ?>
<br clear="all" />
        </div>
      </div>
...
<!-- InstanceEnd --></html>
<?php
((mysqli_free_result($rs_wines) || (is_object($rs_wines) && (get_class($rs_wines) == "mysqli_result"))) ? true : false);
?>
